...
export default function WaitTimes() {
  ...
  useEffect(() => {
    ...
    if (currentTrip.parks.length > 0) {
      const parks = currentTrip.parks
  ...
  const loadAllData = async (isRefresh = false) => {
    ...
    setError("");
    try {
      const [liveAttractions, liveShows, liveCharacterMeets, schedule] =
        await Promise.all([
  ...
  const tabs: DataTab[] = [
    "attractions",
    "shows",
    ...(hasCharacterMeets ? ["characters" as DataTab] : []),
  ];

  const allResorts = PARKS.reduce(
    (acc, park) => {
    ...
    return (
        ...
        actions={[
          {
            ...
            color: favourited
              ? "#FF4D4D20"
              : `${selectedPark?.color ?? theme.accent}20`,
            textColor: favourited
              ? "#FF4D4D"
              : (selectedPark?.color ?? theme.accent),
      ...
      >
        <TouchableOpacity
          ...
          onLongPress={() => {
            haptic();
            toggleFavourite(item.id, "ATTRACTION", item.name);
          }}
        ...
        >
          ...
          {item.status === "OPERATING" ? (
            item.waitTime !== null ? (
              <Text
                style={{ color: selectedPark?.color ?? theme.accent }}
            ...
            <View
              className={`px-2 py-1 rounded-full ${item.status === "DOWN" ? "bg-yellow-500/20" : "bg-border"}`}
            >
              <Text
                className={`text-xs font-medium ${item.status === "DOWN" ? "text-yellow-400" : "text-textSecondary"}`}
              >
    ...
    return (
      ...
      >
        <TouchableOpacity
          ...
          onLongPress={() => {
            haptic();
            toggleFavourite(item.id, item.showType, item.name);
          }}
          style={{ backgroundColor: favBackground(favourited) }}
            ...
            <View className="flex-row flex-wrap gap-2">
              {item.showtimes.map((time, index) => (
                <View
                  key={index}
                  style={{
                    backgroundColor: `${selectedPark?.color ?? theme.accent}20`,
                  }}
        ...
        actions={[
          {
            ...
            textColor: favourited
              ...
              : (selectedPark?.color ?? theme.accent),
            onPress: () =>
              toggleFavourite(item.id, "CHARACTER_MEET", item.name),
          },
        ]}
      >
        <TouchableOpacity
      ...
      <View className="bg-surface rounded-xl px-4 py-3 mb-4">
        <View className="flex-row items-center justify-between mb-1">
          ...
          <Text className="text-text text-xs font-semibold">
            {formatTime(todayHours.openingTime)} –{" "}
            {formatTime(todayHours.closingTime)}
          </Text>
        </View>
        {todayHours.earlyEntry && (
          <View className="flex-row items-center justify-between mb-1">
            ...
            <Text className="text-text text-xs">
              {formatTime(todayHours.earlyEntry.openingTime)} –{" "}
              {formatTime(todayHours.earlyEntry.closingTime)}
            </Text>
          </View>
        )}
        {todayHours.specialEvent && (
          <View className="flex-row items-center justify-between">
            <Text className="text-yellow-400 text-xs">
              {todayHours.specialEvent.description}
            </Text>
            <Text className="text-yellow-400 text-xs">
              {formatTime(todayHours.specialEvent.openingTime)} –{" "}
              {formatTime(todayHours.specialEvent.closingTime)}
            </Text>
          </View>
        )}
      </View>
  ...
  const subtitleText = () => {
    if (activeTab === "attractions")
      return `${operating.length} rides operating · ${lastUpdated ? `updated ${formatTime(lastUpdated.toISOString())}` : "pull to refresh"}`;
          ...
          {tabs.map((tab) => (
            ...
            >
              <Text
                style={{
                  color:
                    activeTab === tab ? theme.background : theme.textSecondary,
                }}
            ...
            <View className="flex-row gap-2">
              {(isBrowsing ? browseParks : tripParks).map((park) => (
                <TouchableOpacity
                  ...
                  style={{
                    backgroundColor:
                      selectedPark?.id === park.id ? park.color : theme.surface,
                  }}
                  className="px-4 py-2 rounded-full"
                >
                  <Text
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        if (isBrowsing) {
                          if (isInPills) {
                            const updated = browseParks.filter(
                              (p) => p.id !== park.id,
                            );
